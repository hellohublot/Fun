...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
	...
	if (offset >= 2 * scrollSinglePageNeedOffset) {
		...
		if (self.delegate && [self.delegate respondsToSelector:@selector(scrollPageViewDidScroll:indexOffset:)]) {
			
			NSInteger currentRow = [[self.displayIndexArray objectAtIndex:1] integerValue];
			CGFloat indexOffset = currentRow + (offset - scrollSinglePageNeedOffset) / scrollSinglePageNeedOffset;
			if (indexOffset < 0) {
				indexOffset = self.displayIndexArray.count + indexOffset;
			}
			[self.delegate scrollPageViewDidScroll:self indexOffset:indexOffset];
...
- (UIButton *)buttonFromIndex:(NSInteger)index {
    __block UIButton *button;
    [self.displayIndexArray enumerateObjectsUsingBlock:^(NSNumber *displayNumber, NSUInteger i, BOOL * _Nonnull stop) {
        if (displayNumber.integerValue == index) {
