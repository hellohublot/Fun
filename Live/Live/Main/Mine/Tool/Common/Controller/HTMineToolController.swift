...
class HTMineToolController: UIViewController {

	let pageModelArray = HTMineToolPageModel.packModelArray()
	
	lazy var navigationBar: HTMineToolNavigationBar = {
		let navigationBar = HTMineToolNavigationBar.init(frame: CGRect.zero)
...
extension HTMineToolController: HTPageHeaderViewDelegate, HTPageContentControllerDelegate {
	...
	func pageContentViewModelDidAppear(_ pageContentView: HTPageContentView, _ pageIndex: Int, _ model: HTPageModelAble?) {
		guard let model = model as? HTMineToolPageModel else {
			return
		}
		if let controller = model.controller as? HTMineToolContentController {
