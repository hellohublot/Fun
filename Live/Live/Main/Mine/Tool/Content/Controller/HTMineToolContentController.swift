//
//  HTMineToolContentController.swift
//  Live
//
...
class HTMineToolContentController: UIViewController {
	...
	func reloadModelArrayWithType(_ type: HTMineToolPageType?) {
		collectionView.setRefreshingBlock(Refresher.circle) { [weak self] (_, _) in
			Request.list.userToolList({ (result) in
				...
				let modelArray = HTMineToolModel.packModelArray(type, value)
...
extension HTMineToolContentController: CollectionViewThrough, UICollectionViewDelegateFlowLayout {
	
	func cellClass<T>(_ reuseView: T, for indexPath: IndexPath) -> ReuseCell.Type where T : ReuseAbleView {
		return HTMineToolContentCell.self
	}
