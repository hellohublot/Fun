//
//  HTMineUserFollowRadioController.swift
//  Live
//
...
class HTMineUserFollowRadioController: UIViewController, HTMineUserFollowControllerAble {
	...
	func initDataSource() {
		tableView.setRefreshingBlock(Refresher.circle) { [weak self] (pageIndex, _) in
			Request.list.userFollowList("1", self?.uidString ?? "", pageIndex - 1, { (result) in
				guard let value = result.value as? [Any] else {
					self?.tableView.endRefreshWith(modelCount: PlaceholderState.errorNetwork.rawValue)
					return
				}
				let modelArray = HTMineUserFollowRadioModel.mj_objectArray(withKeyValuesArray: value) as? [HTMineUserFollowRadioModel] ?? [HTMineUserFollowRadioModel]()
...
extension HTMineUserFollowRadioController: TableViewThrough {
	
	func cellClass<T>(_ reuseView: T, for indexPath: IndexPath) -> ReuseCell.Type where T : ReuseAbleView {
		return HTMineUserFollowRadioCell.self
	}
