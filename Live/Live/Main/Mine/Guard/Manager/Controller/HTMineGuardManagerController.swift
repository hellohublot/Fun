//
//  HTMineGuardManagerController.swift
//  Live
//
...
class HTMineGuardManagerController: UIViewController {
	...
	func initDataSource() {
		tableView.setRefreshingBlock(Refresher.circle) { [weak self] (pageIndex, _) in
			Request.list.userManagerList(pageIndex - 1, { (result) in
				...
				let modelArray = HTMineGuardManagerModel.mj_objectArray(withKeyValuesArray: value["list"]) as? [HTMineGuardManagerModel] ?? [HTMineGuardManagerModel]()
				if pageIndex == 1 {
					self?.tableView.setCellModelArray(modelArray, proxy: self)
				} else {
					var existModelArray = self?.tableView.cellModelArray() as? [HTMineGuardManagerModel] ?? [HTMineGuardManagerModel]()
...
extension HTMineGuardManagerController: TableViewThrough {
	
	func cellClass<T>(_ reuseView: T, for indexPath: IndexPath) -> ReuseCell.Type where T : ReuseAbleView {
		return HTMineGuardManagerCell.self
	}
