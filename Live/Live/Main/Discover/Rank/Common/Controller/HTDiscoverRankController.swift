//
//  HTDiscoverRankController.swift
//  Live
//
...
class HTDiscoverRankController: UIViewController {

	let pageModelArray = HTDiscoverRankPageModel.packModelArray()
	
	lazy var navigationBar: HTDiscoverRankNavigationBar = {
		let navigationBar = HTDiscoverRankNavigationBar.init(frame: CGRect.zero)
...
extension HTDiscoverRankController: HTPageHeaderViewDelegate, HTPageContentControllerDelegate {
	...
	func pageContentViewModelDidAppear(_ pageContentView: HTPageContentView, _ pageIndex: Int, _ model: HTPageModelAble?) {
		guard let model = model as? HTDiscoverRankPageModel, let controller = model.controller as? HTDiscoverRankContentController else {
			return
		}
    ...
    func reloadRankPopSelectedModelWithPageModel(_ pageModel: HTDiscoverRankPageModel, _ controller: HTDiscoverRankContentController) {
        var selectedTitle = HTDiscoverRankPopModel.findSelectedModel(pageModel.rankPopModelArray)?.title ?? ""
        navigationBar.selectedButton.setTitle(selectedTitle, for: .normal)
        controller.reloadContentModelArrayWithPageModel(pageModel)
    }
