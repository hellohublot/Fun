//
//  HTDiscoverSectionHeaderView.swift
//  Live
//
...
class HTDiscoverSectionHeaderView: UICollectionReusableView, ReuseCell {
	...
	lazy var detailButton: UIButton = {
		...
		button.whenTouch(inside: { [weak self] (_, _) in
			let newsController = HTDiscoverNewsController()
			self?.controller?.navigationController?.pushViewController(newsController, animated: true)
		})
		return button
	}()
	...
	func setModel(_ model: Any?, for indexPath: IndexPath) {
		guard let model = model as? HTDiscoverSectionModel else {
			return
		}
		titleButton.setImage(UIImage.init(named: model.image ?? ""), for: .normal)
		titleButton.setTitle(model.title, for: .normal)
		detailButton.isHidden = !(model.showDetailButton ?? false)
	}
