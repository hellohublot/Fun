//
//  HTDiscoverNewsController.swift
//  Live
//
...
class HTDiscoverNewsController: UIViewController {

	let pageModelArray = HTDiscoverNewsPageModel.packModelArray()
	
	lazy var navigationBar: HTDiscoverNewsNavigationBar = {
		let navigationBar = HTDiscoverNewsNavigationBar.init(frame: CGRect.zero)
...
extension HTDiscoverNewsController: HTPageHeaderViewDelegate, HTPageContentControllerDelegate {
	...
	func scrollViewDidScroll(_ scrollView: UIScrollView) {
		...
		fromFrame.origin.x += fromFrame.size.width * 0.35
		fromFrame.size.width *= 0.30
		toFrame.origin.x += toFrame.size.width * 0.35
		toFrame.size.width *= 0.30
	...
	func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, insetForSectionAt section: Int) -> UIEdgeInsets {
		return UIEdgeInsets.init(top: 0, left: 70, bottom: 0, right: 70)
	}
