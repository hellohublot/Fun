//
//  HTLiveHotSectionHeaderView.swift
//  Live
//
...
class HTLiveHotSectionHeaderView: UICollectionReusableView, ReuseCell {
	...
	func setModel(_ model: Any?, for indexPath: IndexPath) {
		guard let model = model as? HTLiveHotSectionModel else {
			return
		}
		titleButton.setImage(UIImage.init(named: model.image ?? ""), for: .normal)
		titleButton.setTitle(model.title, for: .normal)
		detailButton.setImage(UIImage.init(named: model.detailImage ?? ""), for: .normal)
